...
use crate::zmachine::buffer::Buffer;
use crate::zmachine::frame::Frame;
use crate::zmachine::instruction::Branch;
use crate::zmachine::instruction::Instruction;
use crate::zmachine::instruction::Opcode;
use crate::zmachine::instruction::Operand;
use crate::zmachine::instruction::OperandType;
use crate::zmachine::options::Options;
use crate::zmachine::quetzal::QuetzalSave;
...
impl Zmachine {
    pub fn new(data: Vec<u8>, ui: Box<dyn UI + Send>, options: Options) -> Zmachine {
    ...
    #[allow(dead_code)]
    pub fn set_screen_size(&mut self, width: u8, height: u8) {
        // Z-machine header bytes 0x20/0x21: screen width/height in characters.
        // Mirrors `dfrotz -w 80 -h 24` behaviour for the 80×24 VT323 grid.
        if self.memory.len() > 0x21 {
            self.memory.write_byte(0x20, width);
            self.memory.write_byte(0x21, height);
        }
    }
